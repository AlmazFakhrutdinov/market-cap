<template>
  <div class="t-body" @click="$emit('goToCoinInfo', item.id)">
    <div class="t-body__serial-number">
      <label class="t-body__label" @click.stop="$emit('handleCheck', item.id)">
        <input class="t-body__checkbox" type="checkbox" />
        <fa
          v-if="isFavoritesItem"
          class="t-body__icon"
          :icon="['fa', 'star']"
          @click.stop
        />
        <fa v-else class="t-body__icon" :icon="['far', 'star']" @click.stop />
      </label>
    ...
    <div class="t-body__coin-name">
      ...
      <div class="t-body__name">{{ item.name }}</div>
      <div class="t-body__id">
        {{ item.symbol }}
      </div>
    </div>
    <div class="t-body__price">{{ item.current_price }}</div>
    <div
      :style="{
        color:
...
export default {
  ...
  computed: {
    ...
    sparklineUrl() {
      ...
      if (!matches) return
      const imageId = matches.length ? matches[1] : false
      return `https://www.coingecko.com/coins/${imageId}/sparkline`
    }
  },
  methods: {
